...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <article className="transform-gpu rounded-[2rem] border border-slate-200/70 bg-white/80 shadow-2xl shadow-slate-900/10 transition duration-300 hover:-translate-y-1 hover:scale-[1.01] hover:border-cyan-300 hover:shadow-[0_35px_90px_-25px_rgba(56,189,248,0.18)] dark:border-white/10 dark:bg-slate-900/80 dark:shadow-slate-950/40">
      <div className="relative mb-6 h-56 overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-indigo-600 via-purple-700 to-cyan-500" style={imageStyles}>
        <div className="absolute inset-0 bg-slate-950/10 dark:bg-slate-950/20" />
      ...
      <div className="px-6 pb-6">
        <div className="mb-4 flex items-center justify-between gap-4">
          ...
          {project.highlight && (
            <span className="rounded-full bg-cyan-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-200">
              Featured
            </span>
          )}
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {project.tech.map((tech) => (
            <span key={tech} className="glow-badge bg-slate-100/90 text-slate-950 dark:bg-slate-900/80 dark:text-slate-100">
              {tech}
            </span>
          ))}
        </div>
